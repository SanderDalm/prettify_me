...
class AgeRegressor:
    ...
    def __init__(self, height, width):
        ...
        self.prediction = Dense(self.fc2, 1, None)

        self.label = tf.placeholder(tf.int32, [None, 1])
        self.loss = tf.losses.mean_squared_error(labels=self.label, predictions=self.prediction)
    ...
    def train(self, num_steps, batchgen, batch_size, dropout_rate, augment, lr, decay, checkpoint='resnet_model/neural_net'):
        ...
        for step in range(num_steps):
            ...
            if step % 100 == 0:
                ...
                val_loss = self.session.run([self.loss], feed_dict=feed_dict)[0]
    ...
    def predict(self, image):
        ...
        feed_dict = {
            self.x: image.reshape(1, self.height, self.width, 3),
            self.dropout_rate: 0,
            self.augment: 0
        }
